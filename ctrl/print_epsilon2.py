...
for i in range(n):
    for v in fat_tree_graph.edge[i]:
        ...
        captured_packets = rdpcap("/home/wenchen_han_22/results_hardcases/tcpdump_newalgo_10Mbps/{}.pcap".format(veth))
...
for i in range(n):
    ...
    for j in range(len(all_pkts[i]) - 1, -1, -1):
        ...
        if pkt[2].type == TYPE_ALGO_SLOW_RECONS:
            # pdb.set_trace()
            max_cons_time[i] = (pkt[0] - all_pkts[0][0][0] - 4) / 1.008 * 5120 + (100000 - start_ts)
            break
print(max_cons_time)
